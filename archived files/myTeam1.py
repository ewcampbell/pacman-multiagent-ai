...
class AggressiveDefensiveAgent(ReflexCaptureAgent):
    ...
    def getFeatures(self, gameState, action):
        ...
        powerPellets = self.getFoodYouAreDefending(gameState).asList()
        
        # Prioritize stopping invaders
        features['numInvaders'] = len(invaders)
        if invaders:
            invaderDistances = [(self.getMazeDistance(myPos, a.getPosition()), a) for a in invaders]
            invaderDistances.sort()
            closestInvaderDist, closestInvader = invaderDistances[0]
            features['invaderDistance'] = closestInvaderDist
        
        # Guard power pellets if no invaders
        elif powerPellets:
            pelletDistances = [self.getMazeDistance(myPos, p) for p in powerPellets]
            features['powerPelletDistance'] = min(pelletDistances)
        
        # If no threats, go on offense but stay near our side
        else:
            foodList = self.getFood(gameState).asList()
            if foodList:
                foodDistances = [self.getMazeDistance(myPos, f) for f in foodList]
                features['foodDistance'] = min(foodDistances)
